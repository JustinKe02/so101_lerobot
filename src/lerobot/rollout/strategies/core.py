...
from ..stall_guard import StallContactError
...
class RolloutStrategy(abc.ABC):
    ...
    def _teardown_hardware(self, hw: HardwareContext, return_to_initial_position: bool = True) -> None:
        ...
        if self._engine is not None:
            ...
            self._engine.stop()
            inference_failed = self._engine.failed
        abnormal_shutdown = getattr(hw, "abnormal_shutdown", False) or inference_failed
        robot = hw.robot_wrapper.inner
        if robot.is_connected:
            if abnormal_shutdown:
                logger.error(
                    "Abnormal shutdown — skipping return-to-initial-position; robot state is not trusted"
                )
            ...
            if abnormal_shutdown and getattr(robot_config, "disable_torque_on_disconnect", False):
                robot_config.disable_torque_on_disconnect = False
                logger.critical(
                    "Abnormal shutdown: motor torque stays ENABLED so the arm holds its pose instead "
                    "of collapsing. Support the arm, then disable torque manually before handling it."
                )
...
def send_next_action(
    ...
    if stall_guard is not None:
        try:
            stall_guard.observe(processed, sent)
        except StallContactError as exc:
            logger.critical(
                "Stall/contact guard tripped: %s — commanding hold at present position and aborting rollout",
                exc,
            )
            _hold_at_present_position(ctx.hardware.robot_wrapper, processed, obs_raw)
            raise
    return action_dict


def _hold_at_present_position(robot_wrapper, requested: dict, obs_raw: dict) -> None:
    """Re-target the joints from the last dispatch to their observed present positions.

    Replaces a runaway goal (which the safety clamp was chasing) with a neutral
    hold before the rollout aborts, so torque-hold at shutdown does not keep
    pressing into whatever stopped the arm.  Best effort: a robot that cannot
    accept the hold must not mask the original stall error.
    """
    hold = {key: float(obs_raw[key]) for key in requested if isinstance(obs_raw.get(key), (int, float))}
    if not hold:
        logger.error("No present positions available to hold after stall trip")
        return
    try:
        robot_wrapper.send_action(hold)
        logger.info("Hold-at-present position commanded after stall trip: %s", hold)
    except Exception:
        logger.exception("Failed to command hold-at-present position after stall trip")

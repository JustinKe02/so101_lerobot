...
from .base import _TeardownErrors
...
class DAggerStrategy(RolloutStrategy):
    ...
    def teardown(self, ctx: RolloutContext) -> None:
        ...
        errors.run("stop announcement", lambda: log_say("Stopping DAgger recording", play_sounds))

        if self._listener is not None:
            logger.info("Stopping keyboard listener")
            errors.run("keyboard listener shutdown", self._listener.stop)

        hardware_ok, _ = errors.run(
            "hardware cleanup",
        ...
        if self._push_executor is not None:
            ...
            executor = self._push_executor
            self._push_executor = None
            executor_ok, _ = errors.run("push executor shutdown", lambda: executor.shutdown(wait=True))

        finalized_ok = True
        if ctx.data.dataset is not None:
            logger.info("Finalizing dataset...")
            finalized_ok, _ = errors.run("dataset finalization", ctx.data.dataset.finalize)
    ...
    def _run_continuous(self, ctx: RolloutContext) -> None:
        ...
        if ctx.policy.action_filter is not None:
            ctx.policy.action_filter.reset()
        events.reset()
        engine.resume()

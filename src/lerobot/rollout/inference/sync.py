...
class SyncInferenceEngine(InferenceEngine):
    ...
    def __init__(
        ...
        self._max_actions_per_chunk = max_actions_per_chunk
        self._replan_on_clamp = replan_on_clamp
        self._clamp_replan_threshold = clamp_replan_threshold
        self._actions_since_replan = 0
        self._pending_replan_reason: str | None = None
        self._horizon_replans = 0
        self._clamp_replans = 0
        logger.info(
            "SyncInferenceEngine initialized (device=%s, action_keys=%d, "
            "max_actions_per_chunk=%s, replan_on_clamp=%s, clamp_replan_threshold=%.3f)",
            self._device,
            len(ordered_action_keys),
            self._max_actions_per_chunk,
            self._replan_on_clamp,
            self._clamp_replan_threshold,
        )
    ...
    def stop(self) -> None:
        ...
        logger.info(
            "SyncInferenceEngine stopped (horizon_replans=%d, clamp_replans=%d)",
            self._horizon_replans,
            self._clamp_replans,
        )
    ...
    def _replan_if_needed(self) -> None:
        ...
        if reason is None and (
            self._max_actions_per_chunk is not None
            and self._actions_since_replan >= self._max_actions_per_chunk
        ):
            reason = f"execution prefix reached {self._max_actions_per_chunk} actions"
            self._horizon_replans += 1

        if reason is None:
            return

        logger.info("Sync guarded replan: %s", reason)
        self._policy.reset()
        self._actions_since_replan = 0
        self._pending_replan_reason = None
    ...
    def notify_action_result(
        ...
        """Invalidate the remaining sync queue after a material safety clamp."""
        del observation
        if not self._replan_on_clamp or sent is None or self._pending_replan_reason is not None:
            return
        ...
        if not residuals:
            return
        worst_key, worst_residual = max(residuals.items(), key=lambda item: item[1])
        if math.isfinite(worst_residual) and worst_residual <= self._clamp_replan_threshold:
            return

        self._clamp_replans += 1
        self._pending_replan_reason = (
            f"robot safety clamp on {worst_key}: residual={worst_residual:.3f} "
            f"> threshold={self._clamp_replan_threshold:.3f}"
        )
        logger.warning("Sync action queue invalidated: %s", self._pending_replan_reason)

...
class InferenceEngine(abc.ABC):
    ...
    Optional hooks
    --------------
    ``notify_observation`` / ``notify_action_result`` / ``pause`` / ``resume``
    ...
    def notify_action_result(  # noqa: B027
        ...
        """Publish the robot's result for a requested action.  Default: no-op.

        ``sent`` is the action target returned by ``Robot.send_action`` after
        robot-side safety handling. Some robot implementations may return
        ``None`` despite the base robot contract, so callers preserve that
        value rather than assuming the requested action was sent unchanged.
        """

    @contextmanager
    def action_dispatch_guard(self) -> Iterator[bool]:
        """Serialize an action dispatch with fatal-state transitions.

        Synchronous engines have no concurrent fatal transition, so the base
        implementation only reports the current state. Asynchronous engines
        override this method with a shared lock.
        """
        yield not self.failed

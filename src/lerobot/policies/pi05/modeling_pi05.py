...
class PaliGemmaWithExpertModel(
    ...
    def train(self, mode: bool = True):
        super().train(mode)
        if self.freeze_vision_encoder and self.paligemma is not None:
            self.paligemma.model.vision_tower.eval()
        if self.train_expert_only and self.paligemma is not None:
            self.paligemma.eval()
...
class PI05Pytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def __init__(self, config: PI05Config, rtc_processor: RTCProcessor | None = None):
        ...
        self._prefix_cache_backend = None
    ...
    def set_prefix_cache_backend(self, backend) -> None:
        """Use an external prefix prefill backend, such as a TensorRT engine."""
        self._prefix_cache_backend = backend
    ...
    def _compute_prefix_cache(self, images, img_masks, tokens, masks):
        ...
        if self.paligemma_with_expert.paligemma is None:
            raise RuntimeError("PyTorch prefix modules were released without configuring a prefix backend")
    ...
    def sample_actions(
        ...
        **kwargs: Unpack[ActionSelectKwargs],
        ...
        prefix_pad_masks, past_key_values = self._compute_prefix_cache(images, img_masks, tokens, masks)

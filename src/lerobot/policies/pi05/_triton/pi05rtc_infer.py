...
class Pi05RTCInference:
    def __init__(
        ...
        max_prompt_len_override: int | None = None,
        num_steps: int = 10,
    ):
        ...
        if discrete_state_input:
            ...
            if max_prompt_len_override is not None:
                self.max_prompt_len = int(max_prompt_len_override)
                if not 0 < self.max_prompt_len <= self.max_tokenize_len:
                    raise ValueError(
                        "max_prompt_len_override must be in [1, max_tokenize_len], "
                        f"got {self.max_prompt_len} and {self.max_tokenize_len}"
                    )
        ...
        self.weights = {
            ...
            "decoder_time_embeds":                torch.zeros(self.num_steps, 1024,    dtype = torch.bfloat16, device = "cuda"),
        ...
        self.buffers = {
            ...
            "decoder_time_emb":                   torch.empty((self.num_steps, decoder_seq_len, 1024),          dtype = torch.bfloat16, device = "cuda"),
            "decoder_adarms_mod_attn":            torch.empty((self.num_steps, 18, decoder_seq_len, 1024 * 3),  dtype = torch.bfloat16, device = "cuda"),
            "decoder_adarms_mod_ffn":             torch.empty((self.num_steps, 18, decoder_seq_len, 1024 * 3),  dtype = torch.bfloat16, device = "cuda"),
            "decoder_adarms_mod_final":           torch.empty((self.num_steps, decoder_seq_len, 1024 * 3),      dtype = torch.bfloat16, device = "cuda"),
    ...
    def get_decoder_rope_weights(self, prompt_len: int) -> torch.Tensor:
        # LeRobot PI0.5 starts the suffix after every valid encoder token.
        # Reusing the final encoder position shifts every decoder RoPE index by one.
    ...
    def _prepare_prefill(
        ...
        if len(prefill_actions) < prefill_len:
            raise ValueError(
                f"prefill_actions has {len(prefill_actions)} rows but action_prefill_len={prefill_len}"
            )
    ...
    def forward(
        ...
        if prompt_len > self.max_prompt_len:
            raise ValueError(
                f"Tokenized prompt length {prompt_len} exceeds captured capacity {self.max_prompt_len}"
            )

...
class SOFollower(Robot):
    ...
    def configure(self) -> None:
        # USB serial adapters can occasionally drop a status packet while the six motors are configured.
        with self.bus.torque_disabled(num_retry=_MOTOR_COMM_RETRIES):
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        if self.config.max_relative_target is not None:
            present_pos = self.bus.sync_read("Present_Position", num_retry=_MOTOR_COMM_RETRIES)
            goal_present_pos = {key: (g_pos, present_pos[key]) for key, g_pos in goal_pos.items()}
            goal_pos = ensure_safe_goal_position(goal_present_pos, self.config.max_relative_target)

        # Send goal position to the arm
        self.bus.sync_write("Goal_Position", goal_pos, num_retry=_MOTOR_COMM_RETRIES)

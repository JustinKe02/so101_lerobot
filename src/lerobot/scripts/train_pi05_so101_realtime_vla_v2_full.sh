...
readonly RTC_MAX_DELAY="${RTC_MAX_DELAY:-6}"
readonly JOB_NAME="${JOB_NAME:-pi05_so101_realtime_vla_v2_full_10epochs_rtc${RTC_MAX_DELAY}_seed1000}"
...
[[ "$RTC_MAX_DELAY" =~ ^[0-9]+$ ]] || fail "RTC_MAX_DELAY must be a non-negative integer"
(( RTC_MAX_DELAY < 50 )) || fail "RTC_MAX_DELAY must be smaller than the 50-step action chunk"
...
command=(
  ...
  --policy.rtc_training_max_delay="$RTC_MAX_DELAY"

...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    if accelerator.distributed_type == DistributedType.FSDP:
        parameter_dtypes = {param.dtype for param in policy.parameters() if param.is_floating_point()}
        if len(parameter_dtypes) > 1:
            if accelerator.mixed_precision not in {"bf16", "fp16"}:
                raise ValueError(
                    "FSDP cannot flatten mixed-dtype parameters without mixed precision. "
                    f"Found parameter dtypes: {sorted(map(str, parameter_dtypes))}."
                )
            if is_main_process:
                logging.info(
                    "Converting mixed-dtype policy parameters to fp32 before FSDP wrapping; "
                    f"forward/backward compute remains {accelerator.mixed_precision}."
                )

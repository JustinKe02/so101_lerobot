        ...
        if cfg.display_data:
            try:
                ...
                if cleanup_error is None:
                    ...
                    logger.exception("Visualization shutdown also failed")

    engine = ctx.policy.inference if ctx is not None else None
    if engine is not None and engine.failed:
        fatal_error = getattr(engine, "fatal_error", None)
        if run_error is not None:
            logger.error("Inference engine also entered a fatal state: %s", fatal_error)
        else:
            if cleanup_error is not None:
                logger.error("Cleanup also failed after inference fatal: %s", cleanup_error)
            raise RuntimeError("Inference engine failed during rollout") from fatal_error
    if run_error is not None:
        if cleanup_error is not None:
            logger.error("Cleanup also failed after rollout error: %s", cleanup_error)
        raise run_error
    if cleanup_error is not None:
        raise cleanup_error
